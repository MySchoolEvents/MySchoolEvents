...
import { Center, createStyles, Space, Stack, Text, TextInput, Title } from '@mantine/core';
...
function CurrentEventModal({ user, event }: any) {
  ...
  const showStudent = () => {
    ...
    if (emailHead === studentID) {
      return (
        <Stack w="100%">
          <Center>
            <StudentIDCard name={user?.displayName} grade={"12"} studentID={studentID} />
          </Center>
          <Space h="xl" />
          <Center>
            <Button color="gray" radius={"xl"} size="lg" maw="50%" onClick={nextStep}>Continue to next step</Button>
          </Center>
      ...
      <Stepper active={active} onStepClick={setActive} breakpoint="sm" allowNextStepsSelect={false}>
        ...
        <Stepper.Step label="Second step" description="Input Event ID ">
          <Stack h="80vh" spacing={40} pt="30vh">
            ...
            <Center>
              <Text color="dimmed">Note: Ask event administrator for the Event ID</Text>
            </Center>
          </Stack>
